...
| Q29 | MemPalace benchmark claims verification | 🟡 Partially resolved | External claims are now evidence-labeled `unverified` for WeRa operations until WeRa-run reproducibility logs exist. Public repo statements are informative but not sufficient for internal validation baseline. **Owner**: KB steward + technical steward. **Next action**: run reproducible benchmark dry-run and archive logs/results under KB evidence artifacts before upgrading claim status to `verified`. |
...
| Q32 | MemPalace CLI/MCP command compatibility | 🟡 Partially resolved | Environment dry-check indicates required CLIs are not currently installed (`mempalace not found`, `claude not found`), so command blocks remain candidate runbooks and are not executable baseline yet. **Owner**: technical steward. **Next action**: install/validate toolchain, execute smoke tests (`status`, wing listing, search, MCP attach), and store command output evidence before moving to `verified` adoption status. |
