...
| Q29 | MemPalace benchmark claims verification | 🟡 Partially resolved | Stricter reproducibility controls are now implemented (pinned image digest, pinned benchmark commit, pinned dataset SHA256, security gate) and host-run evidence is explicitly non-authoritative (`27`, `28`). External claims remain `unverified` in WeRa operations until a successful container run is completed under this hardened flow. **Current blocker**: Docker daemon unavailable at execution time (`docker_daemon_status=1`). **Owner**: KB steward + technical steward. **Next action**: start Docker daemon, run hardened container benchmark workflow, attach evidence package in PR. |
...
| Q32 | MemPalace CLI/MCP command compatibility | 🟡 Partially resolved | Under the stricter evidence bar, compatibility cannot be promoted to `verified`: local CLIs are currently absent (`mempalace`, `claude` uninstalled) and Docker daemon is not available for containerized validation (`docker_daemon_status=1`). **Owner**: technical steward. **Next action**: reinstall required CLI tooling only if needed for the chosen operating model, start Docker daemon, run containerized smoke-test set, then archive evidence before status promotion. |
