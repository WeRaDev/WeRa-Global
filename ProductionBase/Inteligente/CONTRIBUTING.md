# Contributing to Inteligente

## Branching and pull requests
- Default branch: `main` (versioned in the `WeRa-Global` umbrella repository; `Inteligente` is a `native` project per `ProductionBase/repos.yaml`).
- Use short-lived topic branches (`feat/inteligente-*`, `fix/inteligente-*`, `chore/inteligente-*`). Existing `feat/humancenter-*` branches predating this rename remain valid.
