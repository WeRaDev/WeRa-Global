...
## Purpose
Identify Human Center's cost categories, as input to pricing decisions. These categories apply primarily to Product 3 ("Automation Center"); Shrinking AI (Product 1) has near-zero marginal delivery cost since it is mostly founder time, not infrastructure.

## Scope
- In scope: cost categories expected to apply to Automation Center's build-and-operate automation service.
- Out of scope: an actual quantified cost model (not yet built); Shrinking AI's costs (founder time only, not a meaningful infrastructure cost center).
