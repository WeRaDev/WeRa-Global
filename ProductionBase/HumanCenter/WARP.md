# WARP.md
This file defines project-specific operating rules for the `HumanCenter` sub-project.

## Project intent
Human Center is an AI-automation consultancy (`www.inteligente.site`) that packages and delivers narrowly-scoped AI-driven business-process automations:
- Discover and scope a client's actual workflow bottleneck (Design Thinking / Theory of Constraints framing) before proposing automation.
- Build automations (n8n/Make/Zapier + LLM orchestration, OCR/document extraction) for bounded, high-trust use cases: email triage/drafting/follow-up, document/receipt processing, bookkeeping handoff.
...
- Priority: validate the problem/solution framing with real discovery calls, then scope the first pilot package.
