...
try:
    ...
    class BuilderConfig:  # type: ignore
        """
        Lightweight shim so that importing AsyncClobClient works even when
        py_builder_signing_sdk is unavailable. Any attempt to instantiate this
        class will raise a helpful error prompting the user to install the
        optional dependency.
        """

        def __init__(self, *args, **kwargs):
            raise ModuleNotFoundError(
                "py_builder_signing_sdk is not installed. "
                "Install it to enable builder authentication features."
            )
...
class AsyncClobClient:
    ...
    async def create_order(
        ...
        if options and options.neg_risk:
            ...
            neg_risk = await self.get_neg_risk(order_args.token_id)
...
class SyncClobClient:
    """
    Thin synchronous wrapper for the asynchronous ClobClient.
    Useful for quick scripts or environments that cannot `await`.
    """

    def __init__(self, *args, **kwargs):
        self._client = AsyncClobClient(*args, **kwargs)
    ...
    def close(self):
        return asyncio.run(self._client.aclose())


# Backwards-compatible alias so existing synchronous usage keeps working.
ClobClient = SyncClobClient
