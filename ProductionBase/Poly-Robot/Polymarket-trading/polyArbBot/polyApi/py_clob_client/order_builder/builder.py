try:  # pragma: no cover - optional dependency
    ...
    _HAS_ORDER_UTILS = True
except ModuleNotFoundError:  # pragma: no cover - fallback for minimal installs
    _HAS_ORDER_UTILS = False

    class _MissingOrderUtils:
        def __init__(self, *args, **kwargs):
            raise ModuleNotFoundError(
                "py_order_utils is not installed. Install it to enable order "
                "creation helpers (pip install py-order-utils)."
            )

    UtilsOrderBuilder = _MissingOrderUtils  # type: ignore
    UtilsSigner = _MissingOrderUtils  # type: ignore
    EOA = 0  # default signature type
    UtilsBuy = "BUY"
    UtilsSell = "SELL"
    OrderData = SignedOrder = object  # type: ignore
...
class OrderBuilder:
    ...
    def create_order(
        ...
        """
        if not _HAS_ORDER_UTILS:
            raise ModuleNotFoundError(
                "py_order_utils is required for order creation. "
                "Install it with `pip install py-order-utils`."
            )
