...
async def _get_http_client() -> httpx.AsyncClient:
    """
    Lazily instantiate a shared AsyncClient so connection pooling
    works across the SDK without forcing every caller to manage it.
    """
    global _http_client, _client_lock
    if _client_lock is None:
        # Lazily instantiate the lock inside the running loop
    ...
    async with _client_lock:
        if _http_client is None:
            try:
                ...
                # http2 extras (h2) missing – fall back to HTTP/1.1 so read-only usage works
                _http_client = httpx.AsyncClient(http2=False)
    return _http_client


async def close_http_client() -> None:
    """
    Explicitly close the shared HTTP client. This is optional but helps
    test suites avoid 'unclosed connector' warnings.
    """
    global _http_client
